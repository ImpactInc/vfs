...
public class MysqlPath implements Path {
    ...
    public MysqlPath(MysqlFileSystem fileSystem, byte[] path) {
        this(fileSystem, path, false);
    }
    
    public MysqlPath(MysqlFileSystem fileSystem, byte[] path, boolean normalized) {
    ...
    public MysqlFileSystem getFileSystem() {
        return fileSystem;
    }
    ...
    public MysqlPath getRoot() {
        if (isAbsolute()) {
            return new MysqlPath(fileSystem, new byte[] { this.path[0] });
        }
        return null;
    }
    
    public MysqlPath getFileName() {
        initOffsets();
        ...
        return new MysqlPath(fileSystem, path);
    }
    
    public MysqlPath getParent() {
        initOffsets();
        int nbOffsets = offsets.length;
        if (nbOffsets == 0) {
            return null;
    ...
    public MysqlPath getName(int index) {
        initOffsets();
    ...
    public MysqlPath subpath(int beginIndex, int endIndex) {
        initOffsets();
    ...
    public MysqlPath normalize() {
        byte[] p = getResolved();
        if (p == this.path) {
            return this;
        }
        return new MysqlPath(fileSystem, p, true);
    }

    public MysqlPath resolve(Path other) {
        MysqlPath p1 = this;
        MysqlPath p2 = checkPath(other);
        if (p2.isAbsolute()) {
            return p2;
        ...
        return new MysqlPath(fileSystem, result);
    }

    public MysqlPath resolve(String other) {
    ...
    public MysqlPath relativize(Path other) {
        MysqlPath p1 = this;
        MysqlPath p2 = checkPath(other);
        if (p2.equals(p1)) {
            return new MysqlPath(fileSystem, new byte[0], true);
        }
    ...
    public MysqlPath toAbsolutePath() {
        ...
        return new MysqlPath(fileSystem, result, true);
    }

    public MysqlPath toRealPath(LinkOption... options) throws IOException {
        MysqlPath absolute = new MysqlPath(fileSystem, getResolvedPath()).toAbsolutePath();
    ...
    public int compareTo(Path paramPath) {
        MysqlPath p1 = this;
        MysqlPath p2 = checkPath(paramPath);
        byte[] a1 = p1.path;
        byte[] a2 = p2.path;
        int l1 = a1.length;
        int l2 = a2.length;
    ...
    private MysqlPath checkPath(Path paramPath) {
        if (paramPath == null) {
            throw new NullPointerException();
        }
        if (!(paramPath instanceof MysqlPath)) {
            throw new ProviderMismatchException();
        }
        return (MysqlPath) paramPath;
    }
    ...
    public boolean equals(Object obj) {
        return obj instanceof MysqlPath
                && ((MysqlPath) obj).fileSystem == fileSystem
                && compareTo((MysqlPath) obj) == 0;
    }
    ...
    private static byte[] doGetResolved(MysqlPath p) {
    ...
    private static boolean equalsNameAt(MysqlPath p1, MysqlPath p2, int index)
    {
